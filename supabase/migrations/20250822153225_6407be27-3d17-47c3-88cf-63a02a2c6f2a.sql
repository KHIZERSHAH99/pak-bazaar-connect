-- Fix the constraint issue by removing and recreating it properly
-- First, let's drop the problematic constraint
ALTER TABLE public.audit_logs DROP CONSTRAINT IF EXISTS valid_event_types;

-- Create comprehensive audit logging system for security monitoring
-- Create auth attempts table for security monitoring (if not exists)
...
-- Add rate limiting table for enhanced security (if not exists)
...
-- Indexes for rate limiting (if not exists)
...
-- Add password security events table (if not exists)
...
-- Indexes for password security events (if not exists)
...
-- Add security functions with proper search paths
...
-- Add trigger for updated_at columns (if not exists)
...
-- Clean up old audit logs function
