...
const SidebarNavItems: React.FC<SidebarNavItemsProps> = ({ profile, onLinkClick }) => {
      ...
      {getNavItems().map((item) => (
        <Link
          ...
          className={`flex items-center justify-between px-4 py-3 rounded-md text-sm font-medium group transition-all duration-200 ${
            isActive(item.path)
              ? 'bg-pakistani_green-700 text-white dark:bg-pakistani_green-600'
        ...
        >
          ...
          {item.badge && (
            <Badge variant="info" className="ml-2"> {/* Ensure 'info' variant is styled appropriately */}
              {item.badge}
            </Badge>
          )}
