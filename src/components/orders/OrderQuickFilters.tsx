...
const OrderQuickFilters: React.FC<OrderQuickFiltersProps> = ({
  ...
  const priorityFilters = [{
    ...
    icon: TrendingUp,
    filter: (orders: Order[]) => orders.filter(order => order.requires_attention || order.status === 'pending' && new Date(order.created_at).getTime() < Date.now() - 24 * 60 * 60 * 1000)
  }];
  return <div className="space-y-4 bg-[#aae4aa]/0">
      {/* Main Status Filters */}
      <Card className="bg-[#aae4aa]/0">
        <CardContent className="p-4 rounded-2xl bg-[1#00ff000f] bg-[#00ff00]/0">
          <div className="flex flex-wrap gap-2 bg-[#00ee00]/0">
            {filters.map(filter => {
            const stats = calculateStats(filter.key === 'all' ? undefined : filter.key);
            const Icon = filter.icon;
            const isActive = activeFilter === filter.key;
            return <Button key={filter.key} variant="outline" size="sm" onClick={() => onFilterChange(filter.key)} className={`h-auto p-3 flex flex-col items-start gap-1 min-w-[120px] ${isActive ? filter.activeColor : filter.color} hover:opacity-80 transition-all`}>
                  ...
                  <div className="flex items-center justify-between w-full text-xs">
                    ...
                    {stats.totalValue > 0 && <span className="opacity-75">
      ...
      {userRole === 'wholesaler' && <Card>
          <CardContent className="p-4 bg-[#00ff00]/0">
            ...
            const Icon = filter.icon;
            return <Button key={filter.key} variant="outline" size="sm" onClick={() => onFilterChange(filter.key)} className={`flex items-center gap-2 ${activeFilter === filter.key ? 'bg-orange-600 text-white border-orange-600' : 'bg-orange-50 text-orange-800 border-orange-200 hover:bg-orange-100'}`}>
                    <Icon className="h-4 w-4" />
                    <span className="text-sm">{filter.label}</span>
                    <Badge variant="secondary" className={`text-xs ${activeFilter === filter.key ? 'bg-white/20' : ''}`}>
