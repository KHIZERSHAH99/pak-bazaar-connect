      ...
      <Card className="group overflow-hidden hover:shadow-md transition-all duration-300 border-0 shadow-sm hover:shadow-primary/10 h-full bg-card dark:bg-card">
        ...
        <div className="relative overflow-hidden">
          <img 
            ...
            loading="lazy"
            decoding="async"
            className="w-full h-32 sm:h-48 md:h-52 lg:h-56 xl:h-64 object-cover group-hover:scale-105 transition-transform duration-300"
            onError={(e) => {
        ...
        {/* Product Details */}
        <div className="p-2.5 sm:p-5 lg:p-6 space-y-1.5 sm:space-y-3 flex-1 flex flex-col">
          {/* Product Name */}
          <h3 className="font-semibold text-sm sm:text-base md:text-lg lg:text-xl text-foreground group-hover:text-primary transition-colors font-poppins line-clamp-2 flex-shrink-0">
            {product.name}
          </h3>
          ...
          <div className="space-y-1 sm:space-y-2 flex-shrink-0">
            <div className="flex items-center justify-between">
              <span className="text-base sm:text-xl md:text-2xl lg:text-3xl font-bold text-primary font-poppins">
                PKR {product.price.toLocaleString()}
              </span>
              {product.moq && product.moq > 1 && (
                <Badge variant="outline" className="hidden sm:inline-flex text-xs md:text-sm text-muted-foreground border-border px-2 py-1">
                  MOQ: {product.moq}
                </Badge>
              )}
            </div>
          ...
          {showAddToCart && (
            <div className="mt-auto pt-2 sm:pt-3 flex-shrink-0">
              <Button 
                className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-poppins text-xs sm:text-sm md:text-base h-9 sm:h-10 md:h-12"
                size="default"
              >
