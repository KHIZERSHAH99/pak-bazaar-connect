...
import { getWholesalerAds, pauseAd, resumeAd, type Ad } from '@/lib/ads';
import { TrendingUp, Eye, MousePointer, DollarSign, Play, Pause, BarChart3, Calendar, Target, Plus, Settings, Filter, Download } from 'lucide-react';
  ...
  if (isLoading) {
    return <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => <Card key={i} className="border-0 shadow-sm">
  ...
  return <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        ...
        <div className="flex gap-3">
          ...
          <Button className="bg-pakistani_green-600 hover:bg-pakistani_green-700" onClick={() => setShowCreateDialog(true)}>
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader className="border-b border-gray-100 bg-black/0">
        ...
        <CardContent className="p-0">
          {ads.length === 0 ? <div className="text-center py-12">
              ...
              {ads.map(ad => <div key={ad.id} className="p-6 transition-colors rounded-2xl bg-[#92ff00]/25">
                  <div className="flex items-start justify-between">
                    <div className="flex-1 space-y-4">
                      <div className="flex items-center gap-4">
                        {ad.image && <div className="w-16 h-16 rounded-lg overflow-hidden bg-gray-100">
                            <img src={getImageUrl(ad.image)} alt={ad.headline} className="w-full h-full object-cover" onError={e => {
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        ...
                        <div className="space-y-1 rounded-xl bg-green-50/0">
                      ...
                      <div className="space-y-2">
                        ...
                        <Progress value={ad.budget_cap ? (ad.current_spend || 0) / ad.budget_cap * 100 : 0} className="h-2" />
                      </div>
                    </div>

                    <div className="flex flex-col gap-2 ml-6">
                      {ad.status === 'active' ? <Button variant="outline" size="sm" onClick={() => handlePauseAd(ad.id)} disabled={actionLoading === ad.id}>
                          <Pause className="h-4 w-4 mr-1" />
                          Pause
                        </Button> : ad.status === 'paused' ? <Button variant="outline" size="sm" onClick={() => handleResumeAd(ad.id)} disabled={actionLoading === ad.id} className="text-green-600 border-green-200 hover:bg-green-50">
                          <Play className="h-4 w-4 mr-1" />
                          Resume
                        </Button> : null}
                      
                      <Button variant="ghost" size="sm" className="text-pakistani_green-600 hover:text-pakistani_green-700" onClick={() => handleViewDetails(ad)}>
                        <Eye className="h-4 w-4 mr-1" />
                        View Details
                      </Button>
                      
                      <Button variant="ghost" size="sm" className="text-gray-600 hover:text-gray-700" onClick={() => toast({
                  title: "Settings",
      ...
      {/* Create Ad Dialog */}
      <EnhancedCreateAdDialog isOpen={showCreateDialog} onClose={() => setShowCreateDialog(false)} onAdCreated={() => {
      setShowCreateDialog(false);
      refetch();
    }} />

      {/* Analytics Dashboard */}
      <AdAnalyticsDashboard isOpen={showAnalytics} onClose={() => setShowAnalytics(false)} ad={selectedAd} />
    </div>;
};
