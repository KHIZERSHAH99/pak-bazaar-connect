...
import { TrendingUp, TrendingDown, DollarSign, Package, Clock, CheckCircle, Calendar, Users } from 'lucide-react';
...
const OrderSummaryWidget: React.FC<OrderSummaryWidgetProps> = ({
  ...
  const getOrdersInPeriod = (startDate: Date, endDate?: Date) => {
    return orders.filter(order => {
      ...
      return endDate ? orderDate >= startDate && orderDate < endDate : orderDate >= startDate;
    });
  };
  ...
  const uniqueCustomers = new Set(orders.map(order => userRole === 'wholesaler' ? order.buyer_id : order.shop_id)).size;
  const metrics = [{
  ...
  return <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 mb-6">
      {metrics.map((metric, index) => {
      const Icon = metric.icon;
      return <Card key={index} className="hover:shadow-md transition-shadow">
            <CardContent className="p-4 rounded-2xl bg-[38f1a] bg-[#00ff00]/20">
              <div className="flex items-center justify-between">
                <div className={`p-2 rounded-full ${metric.bgColor}`}>
                  <Icon className={`h-5 w-5 ${metric.color}`} />
                </div>
                
                {metric.change !== null && <Badge variant="secondary" className={`text-xs ${metric.change >= 0 ? 'text-green-700 bg-green-100' : 'text-red-700 bg-red-100'}`}>
