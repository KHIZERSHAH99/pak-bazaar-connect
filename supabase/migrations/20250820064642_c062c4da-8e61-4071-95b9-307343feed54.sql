...
CREATE OR REPLACE FUNCTION public.monitor_sensitive_operations()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  ...
  IF TG_TABLE_NAME IN ('profiles', 'payment_methods', 'orders', 'inquiries') THEN
    PERFORM public.log_sensitive_data_access(
      TG_TABLE_NAME,
      TG_OP,
      CASE 
        WHEN TG_TABLE_NAME = 'profiles' THEN COALESCE(NEW.id, OLD.id)
        WHEN TG_TABLE_NAME = 'payment_methods' THEN COALESCE(NEW.wholesaler_id, OLD.wholesaler_id)
        WHEN TG_TABLE_NAME = 'orders' THEN COALESCE(NEW.buyer_id, OLD.buyer_id)
        WHEN TG_TABLE_NAME = 'inquiries' THEN COALESCE(NEW.buyer_id, OLD.buyer_id)
        ELSE NULL
      END
    );
  END IF;
...
DROP TRIGGER IF EXISTS monitor_payment_methods_access ON public.payment_methods;
DROP TRIGGER IF EXISTS monitor_inquiries_access ON public.inquiries;

CREATE TRIGGER monitor_profiles_access
  AFTER SELECT OR UPDATE ON public.profiles
  FOR EACH ROW
  EXECUTE FUNCTION public.monitor_sensitive_operations();

CREATE TRIGGER monitor_payment_methods_access
  AFTER SELECT ON public.payment_methods
  FOR EACH ROW
  EXECUTE FUNCTION public.monitor_sensitive_operations();

CREATE TRIGGER monitor_inquiries_access
  AFTER SELECT ON public.inquiries
  FOR EACH ROW
  EXECUTE FUNCTION public.monitor_sensitive_operations();