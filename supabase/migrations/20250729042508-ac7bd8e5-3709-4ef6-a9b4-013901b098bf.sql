-- First create the order_status enum type if it doesn't exist
DO $$ 
BEGIN
    -- Check if the enum exists, if not create it
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'order_status') THEN
        CREATE TYPE order_status AS ENUM ('pending', 'confirmed', 'completed', 'rejected', 'cancelled');
    END IF;
END $$;

-- Now add the new status values
DO $$
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'packed' AND enumtypid = (SELECT oid FROM pg_type WHERE typname = 'order_status')) THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'delivered' AND enumtypid = (SELECT oid FROM pg_type WHERE typname = 'order_status')) THEN
