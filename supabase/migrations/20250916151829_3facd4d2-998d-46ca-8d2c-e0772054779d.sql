...
-- Add SET search_path = public to all functions to prevent search_path attacks
...
ALTER FUNCTION public.send_otp_sms(text, text) SET search_path = public;

-- 2. Create secure views for sensitive data
-- ==========================================

-- Create a secure view for profiles that masks sensitive data
...
SELECT 
    ...
    CASE 
        WHEN auth.uid() = id OR get_user_role() = 'admin' THEN email
        ELSE NULL
    END as email,
    CASE 
        WHEN auth.uid() = id OR get_user_role() = 'admin' THEN phone_number
        ELSE NULL
    END as phone_number,
    CASE 
        WHEN auth.uid() = id OR get_user_role() = 'admin' THEN address
        ELSE NULL
    END as address,
    CASE 
        WHEN auth.uid() = id OR get_user_role() = 'admin' THEN postal_code
        ELSE NULL
    END as postal_code,
    -- Never expose verification documents
    CASE 
        WHEN cnic_image IS NOT NULL THEN '[DOCUMENT ON FILE]'
        ELSE NULL
    END as cnic_status,
    CASE 
        WHEN selfie_image IS NOT NULL THEN '[IMAGE ON FILE]'
        ELSE NULL
    END as selfie_status
FROM public.profiles;

-- Grant appropriate access
GRANT SELECT ON public.profiles_public TO authenticated;

-- 3. Enhance payment methods security
-- ====================================

-- Add encryption columns for sensitive payment data
...
-- Create function to encrypt payment data
...
BEGIN
    ...
    IF NEW.account_number IS DISTINCT FROM OLD.account_number AND NEW.account_number IS NOT NULL THEN
        NEW.account_number_encrypted := pgp_sym_encrypt(NEW.account_number, encryption_key);
        NEW.account_number_masked := mask_sensitive_data(NEW.account_number, 'account');
        NEW.account_number := NULL; -- Clear plain text
    END IF;
    
    IF NEW.jazzcash_number IS DISTINCT FROM OLD.jazzcash_number AND NEW.jazzcash_number IS NOT NULL THEN
        NEW.jazzcash_encrypted := pgp_sym_encrypt(NEW.jazzcash_number, encryption_key);
        NEW.jazzcash_masked := mask_sensitive_data(NEW.jazzcash_number, 'phone');
        NEW.jazzcash_number := NULL; -- Clear plain text
    END IF;
    
    IF NEW.easypaisa_number IS DISTINCT FROM OLD.easypaisa_number AND NEW.easypaisa_number IS NOT NULL THEN
        NEW.easypaisa_encrypted := pgp_sym_encrypt(NEW.easypaisa_number, encryption_key);
        NEW.easypaisa_masked := mask_sensitive_data(NEW.easypaisa_number, 'phone');
        NEW.easypaisa_number := NULL; -- Clear plain text
    END IF;
...
-- Create enhanced audit log trigger for sensitive operations
...
-- 5. Strengthen RLS policies for critical tables
-- ===============================================

-- Fix profiles table RLS to prevent data leakage
...
-- 7. Create security monitoring dashboard view
-- ============================================
...
AND tablename NOT IN ('schema_migrations', 'security_metrics')
AND NOT EXISTS (
...
UNION ALL
SELECT 
    'Failed login attempts (last hour)' as metric,
    COUNT(*) as value
FROM public.audit_logs
WHERE event_type IN ('login_failed', 'auth_failed')
AND created_at > now() - interval '1 hour'
UNION ALL
SELECT 
    'Suspicious activities (last 24h)' as metric,
    COUNT(*) as value
FROM public.audit_logs
...
UNION ALL
SELECT 
    'Admin actions (last 24h)' as metric,
    COUNT(*) as value
FROM public.audit_logs
WHERE user_id IN (
...
-- 8. Clean up old sensitive data automatically
-- ============================================

-- Schedule cleanup of old payment screenshots
